...
import { getFirestore, collection, getDocs, updateDoc, getDoc, addDoc, doc, setDoc, arrayUnion, arrayRemove, FieldValue, deleteDoc } from "firebase/firestore";
...
export const useToDo = defineStore({
    ...
    actions: {
        ...
        async updateTitleTCLI(id_item, id_item_item) {
            let title = this.todo.checklist.list[id_item].items[id_item_item].title_item;
        ...
        async updateCheckedTCLI(id_item, id_item_item) {
            let checked = this.todo.checklist.list[id_item].items[id_item_item].checked;
            let arr = this.todo.checklist.list[id_item].items;
        ...
        async deleteWLI(id_item, day) {
            ...
            if(day == "Monday"){
                if(list_day.monday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.monday[id_item].id.toString()));
                    list_day.monday.splice(id_item, 1);
                } else {
                    let arr = list_day.monday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.monday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }else if(day == "Tuesday"){
                if(list_day.tuesday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.tuesday[id_item].id.toString()));
                    list_day.tuesday.splice(id_item, 1);
                } else {
                    let arr = list_day.tuesday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.tuesday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }else if(day == "Wednesday"){
                if(list_day.wednesday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.wednesday[id_item].id.toString()));
                    list_day.wednesday.splice(id_item, 1);
                } else {
                    let arr = list_day.wednesday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.wednesday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }else if(day == "Thursday"){
                if(list_day.thursday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.thursday[id_item].id.toString()));
                    list_day.thursday.splice(id_item, 1);
                } else {
                    let arr = list_day.thursday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.thursday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }else if(day == "Friday"){
                if(list_day.friday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.friday[id_item].id.toString()));
                    list_day.friday.splice(id_item, 1);
                } else {
                    let arr = list_day.friday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.friday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }else if(day == "Saturday"){
                if(list_day.saturday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.saturday[id_item].id.toString()));
                    list_day.saturday.splice(id_item, 1);
                } else {
                    let arr = list_day.saturday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.saturday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }else if(day == "Sunday"){
                if(list_day.sunday[id_item].day.length == 1) {
                    await deleteDoc(doc(db, "weekly_list", list_day.sunday[id_item].id.toString()));
                    list_day.sunday.splice(id_item, 1);
                } else {
                    let arr = list_day.sunday[id_item].day;
                    arr.splice(arr.indexOf(day), 1);
                    await updateDoc(doc(db, "weekly_list", list_day.sunday[id_item].id.toString()), {
                        day: arr,
                    });
                }
            }
